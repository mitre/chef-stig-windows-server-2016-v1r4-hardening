...
powershell_script 'V-73405' do
 
  code <<-EOH
  $systemroot = $Env:SYSTEMROOT
  $FolderPath = "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\Application.evtx"
 
  $ACL = (Get-Item $FolderPath).GetAccessControl('Access')

  $NonCompliantRules = $ACL.Access | Where-Object {
  $_.IdentityReference -notlike "*Administrators*" -and $_.IdentityReference -notlike "*Eventlog*" -and $_.IdentityReference -notlike "*SYSTEM*"
  }
  ...
  $acl | Set-Acl -Path "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\Application.evtx"

  EOH
end

powershell_script '73407' do
 
  code <<-EOH
  $systemroot = $Env:SYSTEMROOT
  $FolderPath = "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\Security.evtx"
  ...
  $acl | Set-Acl -Path "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\Security.evtx"

  EOH
end

powershell_script '73409' do
 
  code <<-EOH
  $systemroot = $Env:SYSTEMROOT
  $FolderPath = "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\System.evtx"
  ...
  $acl | Set-Acl -Path "$systemroot\\SYSTEM32\\WINEVT\\LOGS\\System.evtx"
